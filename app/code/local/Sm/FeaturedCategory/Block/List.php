...
-------------------------------------------------------------------------*/
class Sm_FeaturedCategory_Block_List extends Mage_Catalog_Block_Product_Abstract
{
    ...
    public function _getCfg($attr = null)
    {
        ...
        $defaults = array();
        $def_cfgs = Mage::getConfig()->loadModulesConfiguration('config.xml')->getNode('default/featuredcategory_cfg')->asArray();
        ...
        $_configs = (array)Mage::getStoreConfig("featuredcategory_cfg");
        if (empty($_configs)) return;
        $cfgs = array();
        foreach ($groups as $group) {
            $_cfgs = Mage::getStoreConfig('featuredcategory_cfg/' . $group . '');
    ...
    {
        if (!$this->_getConfig('active', 1)) return;
        $template_file = "sm/featuredcategory/default.phtml";
    ...
    public function _getProductInfor($_catids, $count_product = false)
    {
        !is_array($_catids) && settype($_catids, 'array');
        if (!empty($_catids)) {
            $products = $this->_getProductsBasic($_catids, $count_product);
            if ($products != null) {
                ...
                if (!empty($_products)) {
                    ...
                    foreach ($_products as $_product) {
                        ...
                        $_product->short_description = $_product->getShortDescription();
    ...
    public function _getCatinfor($catids)
    {
        ...
        if (!empty($catids)) {
            ...
            foreach ($catids as $catid) {
                ...
                $category->_description = $helper->_trimEncode($category->description != '') ? $helper->truncate($category->_description, $this->_getConfig('category_desc_max_characs', 5)) : '';
                $products = $this->_getProductInfor($category->id, false);
    ...
    {
        return md5(serialize(array('sm_featuredcategory', $this->_config)));
    }
    ...
    public function _getList()
    {
        ...
        return $this->_getCatinfor($_catids);
    }

    public function _countProductsBasic($catids, $limit = 0)
    {
        $collection = array();
        !is_array($catids) && settype($catids, 'array');
        if (!empty($catids)) {
            ...
            if ($limit >= 0) {
                $collection->getSelect()->limit($limit, $_start);
            }
        }
        return $collection;
    }
