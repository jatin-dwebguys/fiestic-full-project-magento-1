...
-------------------------------------------------------------------------*/
class Sm_FeaturedCategory_Helper_Data extends Mage_Core_Helper_Abstract
{
    protected $_config = null;

    public function getInlucdeJQquery()
    {
        if (!(int)Mage::getStoreConfig('featuredcategory_cfg/general/active')) return;
        if (!defined('MAGENTECH_JQUERY') && (int)Mage::getStoreConfig('featuredcategory_cfg/advanced/include_jquery')) {
            define('MAGENTECH_JQUERY', 1);
            $_jquery_libary = 'sm/categoriesfeutured/js/jquery_1.11.3.js';
    ...
    {
        if (!(int)Mage::getStoreConfig('featuredcategory_cfg/general/active')) return;
        if (!defined('MAGENTECH_JQUERY_NOCONFLICT') && (int)Mage::getStoreConfig('featuredcategory_cfg/advanced/include_jquery')) {
            define('MAGENTECH_JQUERY_NOCONFLICT', 1);
            $_jquery_noconflict = 'sm/categoriesfeutured/js/jquery-noconflict.js';
            ...
            foreach ($mark as $type => $true) {
                switch ($type) {
                    ...
                    case 'product_description':
						$text = (Mage::getModel('catalog/product')->load($product->getId())->getDescription() == '')?$product->short_description:Mage::getModel('catalog/product')->load($product->getId())->getDescription();
						$inline_images = $this->getInlineImages($text);
                        if (!empty($inline_images)) {
                            for ($i = 0; $i < count($inline_images); $i++) {
								if(strlen(strstr($inline_images[$i]['src'], 'media url=')) > 0){
									
                                    $array1 = explode('url=',$inline_images[$i]['src']);
									$array2 = explode('}}',$array1[1]);
									$_url_media = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$array2[0];
									
									$url_media = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$array2[0];
									$url_media1 = explode("/'", $url_media);
									$url_media2 = explode('/"', $url_media);
									if(count($url_media1) == 2){
										$url2 = explode("'",$url_media1[1]);
										$url = $url_media1[0].'/'.$url2[0];
									}elseif (count($url_media2) == 2){
										$url3 = explode('"',$url_media2[1]);
										$url = $url_media2[0].'/'.$url3[0];
									}else{
										$url = $_url_media;
									}
									if (file_exists($url) || @getimagesize($url) !== false){
										array_push($images_path, $url);
									}
                                }
                            }
                        }
    ...
    public function getCatImage($category, $prefix = 'imgcfgcat')
    {
        $images = $this->getCatImages($category, $prefix);
        return is_array($images) && count($images) ? $images[0] : null;
    }
